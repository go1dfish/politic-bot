...
function main(config, botTasks) {
  ...
  api.on('access_token_expired', wrap(function() {
    return main.schedule.retry(function() {return api.auth();}, function(e) {return true;});
  }));
  ...
  return api.auth().then(function() {
    bot = {
      ...
      duplicates: wrap(function(post) {
        ...
        return api('/duplicates/' + post.id).get({limit:100}).then(function(results) {
      ...
      submit: wrap(function(postData) {
        ...
        if (!_.contains(bot.subs.concat(['stuff']), (postData.sr||'').toLowerCase())) {
