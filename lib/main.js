  ...
  return api.auth().then(function() {
    ...
    api.on('access_token_expired', wrap(function() {
      return new RSVP.Promise(function(resolve, reject) {
        try {
          bot.ingest.saveDatabase(function(error) {
            if (error) {retject(error);} else {resolve(bot.ingest);}
          });
        } catch(e) {console.error('save error', e.stack || e);}
      }).catch(function(e) {
        console.error('auth', e.stack || e);
      }).finally(function() {
        return schedule.wait(30).then(function() {return api.auth();});
      }).catch(function(e) {
        console.error('auth 2', e.stack || e);
      }).then(function() {
        return schedule.retry(function() {return api.auth();}, function(e) {
